...
@section('container')
  ...
  <form method="post" action="{{ route('verifyPurchaseCode') }}" class="tabs-wrap">
    ...
    <div class="form-group {{ $errors->has('name') ? ' has-error ' : '' }}">
      ...
      <input type="text" name="name" id="name"  placeholder="{{ trans('installer_messages.purchase_code.buyer_name_placeholder') }}" required />
      @if ($errors->has('name'))
    ...
    <div class="form-group {{ $errors->has('email') ? ' has-error ' : '' }}">
      ...
      <input type="text" name="email" id="email"  placeholder="{{ trans('installer_messages.purchase_code.buyer_email_placeholder') }}" required />
      @if ($errors->has('email'))
          <span class="error-block">
              <i class="fa fa-fw fa-exclamation-triangle" aria-hidden="true"></i>
              {{ $errors->first('email') }}
