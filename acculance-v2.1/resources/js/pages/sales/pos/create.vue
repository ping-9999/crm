        ...
        <div class="card">
          <div class="card-body-l p-0">
            <div class="card-client-search">
              <div>
                <v-select v-model="form.client" :options="items ? items : []" label="name"
            ...
            <div class="row pt-3 pl-3 pr-3">
              ...
              <div class="form-group col-md-6 col-lg-6">
                ...
                <input id="transportCost" v-model="form.transportCost" type="number" step="any" min="1"
                       ...
                       name="transportCost"
                       :placeholder="$t('common.transport_cost_placeholder')" @change="calculateSum"
                       @keyup="calculateSum"/>
        ...
        <div class="card bg-transparent">
          <div class="pos-r-head bg-white">
            <div class="row">
              ...
              <div v-if="products" class="form-group col-md-12 mb-0">
                <div class="mb-2">
                  <search :isPosSearch="true" v-model="query" @reset-pagination="resetPagination()" @reload="reload"/>
                </div>
          ...
          <div class="card-body bg-white mt-3 pos-body">
            <div class="pos-item-grid">
              <div v-for="product in products" :key="product.id" @click="storeProduct(product)" :class="
          ...
          <div style="max-width:400px;margin:0px auto">
            ...
            <table class="table_data">
              ...
              <tr style="margin-top:10px">
                ...
                <td style="text-align:right;" class="total">{{ allData.subTotal | withCurrency }}</td>
              </tr>
              <tr v-if="allData.discount" style="margin-top:10px">
                <td colspan="3" class="total">{{ $t('common.discount') }}</td>
                <td style="text-align:right;" class="total">{{ allData.discount | withCurrency }}</td>
              </tr>
              <tr v-if="allData.tax" style="margin-top:10px">
                <td colspan="3" class="total">{{ $t('common.tax') }}(%)</td>
                <td style="text-align:right;" class="total">{{ allData.tax | withCurrency }}</td>
              </tr>
              <tr style="margin-top:10px">
              ...
              <tr>
                ...
                <td style="text-align:right;" class="total">{{ allData.totalPaid | withCurrency }}</td>
              </tr>
              <tr>
                <td colspan="3" class="total">{{ $t('common.due') }}</td>
                <td style="text-align:right;" class="total">{{ allData.due | withCurrency }}</td>
              </tr>
              </tbody>
            </table>
...
export default {
  ...
  mounted() {
    window.addEventListener("keypress", e => {
      if (this.form.netTotal > 0 && this.showModal == false && this.generateOrder == false) {
        if (e.key === 'Enter') {
          this.generateOrder = true
          this.completeOrderAndAddPayment()
        }
      }
      if (this.form.netTotal > 0 && this.form.paidAmount > 0 && this.generateOrder == true) {
        if (e.key === 'Enter') {
          this.clickCount++
  ...
  methods: {
    ...
    async getTaxes() {
      ...
      if (this.taxes && this.taxes.length > 0) {
        ...
        this.form.orderTax = this.taxes.find(item => item.slug === defaultVatRateSlug)
      }
    },
    ...
    async getProducts() {
      ...
      const {data} = await this.form.get(
        window.location.origin + '/api/all-products?page=' + currentPage
      )
